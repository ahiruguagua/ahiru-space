...
const LEADERBOARD_KEY = 'duck-life-leaderboard';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { name, score, job, rank } = req.body;
      ...
      scores.push({
        ...
        job: typeof job === 'string' ? job.slice(0, 30) : '',
        rank: typeof rank === 'string' ? rank.slice(0, 30) : '',
